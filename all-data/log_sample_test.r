...
tl_matrix <- remove_na(log(acast(t3,cust_no~nth,value.var="travel_length")+1),-1)
nth_matrix <- remove_na(log(acast(t3,cust_no~nth,value.var="nth")),-1)
package_amt_matrix <- remove_na(log(acast(t3,cust_no~nth,value.var="package_amt")+1),-1) # too make -Inf to 0
...
result_theta_age2 <- stan('./20141201-theta=age,tl,nth-phi=accos,tl,pkg_amt,age,freq.stan',data=stan_semisup_region,iter=200,chains=1,init=0)
...
result_phi_accos_np_thres <- stan('./phi_accos_no_prior_thres.stan',data=stan_semisup_region,iter=200,chains=1,init=0)
...
restricted_patpa <- stan('./restricted_patpa.stan',data=stan_semisup_region,iter=200,chains=1,init=0)
